...
export default class DFController {
        ...
        if (req.method == "POST" && req.is("application/json")) {
            ...
            switch(intencao) {
                case 'Default Welcome Intent':
                    ...
                    break
                case `Consultar protocolo`:
                    resposta = await this.consultarAtendimentoPorProtocolo(origem, dados)
                    break
    ...
    async consultarAtendimentoPorProtocolo(origem, dados) {

        const protocolo = dados.queryResult.parameters['protocolo']

        const atenDAO = new AtendimentoDAO();
        const atendimento = await atenDAO.buscarPorProtocolo(protocolo);
    
        if (!atendimento) {
            return `Desculpe, não encontramos nenhum atendimento com o protocolo ${protocolo}.`;
        }
    
        const { nome, produto } = atendimento;
